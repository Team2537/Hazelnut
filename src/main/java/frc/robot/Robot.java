...
public class Robot extends TimedRobot {
  ...
  public void robotInit() {
    ...
    r_ultrasonic = new Ultrasonic(1,0); // ping, echo
  ...
  public void teleopPeriodic() {
    ...
    if (f_ultrasonic.isRangeValid()) {
       if ((f_range <= 18.0) && !f_safetyStop) {
          System.out.println("Safety stopped due to front obstacle: "+f_range);
          f_safetyStop = true;
       } else if (f_range >= 20.0 && f_safetyStop) {
          System.out.println("Resuming...");
          f_safetyStop = false;
       }
    }

    // Use rear-mounted ultrasonic sensor to stop robot
    // if it gets too close to an obstacle
    double r_range = r_ultrasonic.getRangeInches();
    if (r_ultrasonic.isRangeValid()) {
       if ((r_range < 18.0) && !r_safetyStop) {
          System.out.println("Safety stopped due to rear obstacle: "+r_range);
          r_safetyStop = true;
       } else if (r_range >= 20.0 && r_safetyStop) {
          System.out.println("Resuming: "+r_range);
          r_safetyStop = false;
       }
    }
